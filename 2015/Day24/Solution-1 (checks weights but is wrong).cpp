...
bool find_group_helper(
    ...
    if (group.size() > min_size && layer == 1) {
        return false;
    }
    ...
    if (target == present) {
        ...
        if (find_group_helper(presents, vector<int>(), s_target, s_target, 0, layer + 1, min_size, min_entanglement)) {
            if (layer == 1) {
    ...
    for(int i = idx; i < presents.size(); i++) {
        vaild = vaild || find_group_helper(presents, group, s_target, target, i, layer, min_size, min_entanglement);
    }
    return vaild;
}
...
size_t quantum_entanglement(vector<int> presents) {
    ...
    for(int i = 0; i < presents.size(); i++) {
        find_group_helper(presents, vector<int>(), target, target, i, 1, min_size, min_entanglement);
    }
