...
void simply_distances_recsurive(
    ...
    for(auto loc : memo) {
        simply_distances_recsurive(distance_memo, loc.first, max_distance, distance + loc.second);
    }
}
size_t simply_distances(unordered_map<string, unordered_map<string, int>> distance_memo) {
    ...
    for(auto loc : distance_memo) {
        simply_distances_recsurive(distance_memo, loc.first, max_distance, 0);
        //cout << endl;
    }
...
int main() {
    ...
    cout << "The longest route is " << output << endl;
}