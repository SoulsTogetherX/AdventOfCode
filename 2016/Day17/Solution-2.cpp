...
void find_path_recusive(Point2D curr, string path, unsigned int distance, unsigned int &max_distance) {
    if (curr == END) {
    ...
    if (in_bounds(next) && is_door_open(md5[UP])) {
        find_path_recusive(next, path + "U", distance, max_distance);
    }
    next = curr + Point2D(0, 1);
    if (in_bounds(next) && is_door_open(md5[DOWN])) {
        find_path_recusive(next, path + "D", distance, max_distance);
    }
    next = curr + Point2D(-1, 0);
    if (in_bounds(next) && is_door_open(md5[LEFT])) {
        find_path_recusive(next, path + "L", distance, max_distance);
    }
    next = curr + Point2D(1, 0);
    if (in_bounds(next) && is_door_open(md5[RIGHT])) {
        find_path_recusive(next, path + "R", distance, max_distance);
    }
}

unsigned int find_longest_path() {
    unsigned int max_distance = 0;

    find_path_recusive(START, SECERT, 0, max_distance);
    return max_distance;
}
...
int main() {
    ...
    auto output = find_longest_path();
    cout << "The longest path's length is " << output << endl;
}