...
const int HEX_POS_OFFSET = 5; // pos - 1
const int HEX_CHAR_OFFSET = 6; // pos - 1
...
void find_next_five_zero(size_t &st, char &character, int &pos) {
    string secert;
    ...
    for(size_t i = st;; i++) {
        ...
        if (md_len >= ZERO_REQUIREMENT && check_zeros(secert)) {
            st = i + 1;
            cout << "found character: " << secert[HEX_CHAR_OFFSET] << " at pos " << secert[HEX_POS_OFFSET] << " - " << secert << endl;
            
            pos = secert[HEX_POS_OFFSET] - '0';
            character = secert[HEX_CHAR_OFFSET];
            return;
        }
...
string find_password() {
    ...
    while(!check_password(password)) {
        find_next_five_zero(st, character, pos);

        if (0 <= pos && pos < PASSWORD_LENGTH && password[pos] == '-') {
